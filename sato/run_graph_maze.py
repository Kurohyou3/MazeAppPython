# coding: utf-8
"""
このスクリプトは、ノートブック版 ``graph_maze.ipynb`` の処理を
コマンドラインから実行できるようにしたものです。迷路を自動生成して
``sato/map_data`` フォルダに JSON 形式で保存します。
...
上記のコマンドでは迷路を 1 つ生成します。生成結果は
``sato/map_data/maze_10.json`` に保存されます
（"10" は迷路のサイズを表します）。

## よく使うオプション

* ``-n`` または ``--count``  – 生成する迷路の数。
* ``-w`` または ``--workers`` – 同時に動かすプロセス数。
  プロセスはコンピューターが処理を行う単位だと思ってください。
* ``--seed`` – 乱数シードを固定します。値を指定すると毎回同じ迷路が
  得られます。乱数シードとは乱数を作る際の "初期値" のようなものです。

### バックグラウンド実行例

接続が切れても処理を続けたい場合は ``nohup`` を使います。

```
nohup python3 run_graph_maze.py -n 10 -w 4 > log.txt 2>&1 &
```

``log.txt`` に実行ログが保存され、端末を閉じても処理が続行されます。
"""
